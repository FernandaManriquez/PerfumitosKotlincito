...
import com.miapp.xanostorekotlin.ui.fragments.PendingOrdersFragment
...
class HomeActivity : AppCompatActivity() { // Declaramos la Activity Home, que gestiona los fragments
    ...
    override fun onCreate(savedInstanceState: Bundle?) { // Métodoo de ciclo de vida: se llama al crear la Activity
        ...
        val isAdmin = tokenManager.getUserRole()?.lowercase() == "admin"
        val addItem = binding.bottomNav.menu.findItem(com.miapp.xanostorekotlin.R.id.nav_add)
        addItem.isVisible = isAdmin
        val usersItem = binding.bottomNav.menu.findItem(com.miapp.xanostorekotlin.R.id.nav_users)
        usersItem.isVisible = isAdmin
        val ordersItem = binding.bottomNav.menu.findItem(com.miapp.xanostorekotlin.R.id.nav_orders)
        ordersItem.isVisible = isAdmin
        val cartItem = binding.bottomNav.menu.findItem(com.miapp.xanostorekotlin.R.id.nav_cart)
        cartItem.isVisible = !isAdmin
        ...
        binding.bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) { // Decidimos qué fragment mostrar según el ítem
                ...
                com.miapp.xanostorekotlin.R.id.nav_cart -> replaceFragment(CartFragment())
                com.miapp.xanostorekotlin.R.id.nav_add -> {
                    if (isAdmin) replaceFragment(AddProductFragment())
                }
                com.miapp.xanostorekotlin.R.id.nav_users -> {
                    if (isAdmin) replaceFragment(UsersFragment())
                }
                com.miapp.xanostorekotlin.R.id.nav_orders -> {
                    if (isAdmin) replaceFragment(PendingOrdersFragment())
                }
            }
