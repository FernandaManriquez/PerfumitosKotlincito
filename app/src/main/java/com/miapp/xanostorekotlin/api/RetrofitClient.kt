...
object RetrofitClient { // Objeto singleton que expone métodos de fábrica
        ...
        return OkHttpClient.Builder() // Iniciamos el builder de OkHttp
            ...
            .addInterceptor { chain ->
                val req = chain.request().newBuilder()
                    ...
                    .addHeader("User-Agent", "XanoStoreKotlin/1.0 Android")
                    .build()
                chain.proceed(req)
            }
    ...
    fun createUserServiceOnStore(context: Context): UserService {
        val tokenManager = TokenManager(context)
        val client = baseOkHttpBuilder()
            .addInterceptor(AuthInterceptor { tokenManager.getToken() })
            .build()
    ...
    fun createCartProductService(context: Context): CartProductService {
        val tokenManager = TokenManager(context)
        val client = baseOkHttpBuilder()
            .addInterceptor(AuthInterceptor { tokenManager.getToken() })
            .build()
        return retrofit(storeBaseUrl, client).create(CartProductService::class.java)
    }
    ...
    fun createShippingService(context: Context): ShippingService {
        val tokenManager = TokenManager(context)
        val client = baseOkHttpBuilder()
            .addInterceptor(AuthInterceptor { tokenManager.getToken() })
            .build()
        return retrofit(storeBaseUrl, client).create(ShippingService::class.java)
    }
}
